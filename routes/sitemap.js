...
router.get('/sitemap.xml', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query('SELECT id, slug, updated_at, created_at FROM posts ORDER BY created_at DESC;');
    ...
    xml += `  <url>\n    <loc>${frontendHost}/login</loc>\n    <changefreq>monthly</changefreq>\n    <priority>0.3</priority>\n  </url>\n`;

    if (posts && posts.length > 0) {
      posts.forEach(post => {
        const identifier = post.slug || post.id;
        const rawDate = post.updated_at || post.created_at;
        const lastModDate = rawDate ? new Date(rawDate).toISOString() : new Date().toISOString();

        xml += `  <url>\n`;
        xml += `    <loc>${frontendHost}/posts/${identifier}</loc>\n`;
